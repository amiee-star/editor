...
const _VideoEditPanel: React.FC<Props> = props => {
	const Intl = useIntl()
	const { data } = props
			...
			<Space className="full-w" direction="vertical">
				...
				<Space className="full-w" direction="vertical">
					...
					<ItemCheckBox
						item={data.extdata.info.custom}
						valueKey="inlineplay"
						children={<FormattedMessage id="jmk.addmaterial.Playonwall" />}
						forceUpdate={forceUpdate}
					/>
					<ItemCheckBox
						item={data.extdata.info.custom}
				...
				<Space className="full-w" direction="vertical">
					...
					<div>
						<ItemSlider item={data} valueKey="opacity" min={0.000001} max={1} step={0.000001} />
					</div>
				</Space>
			</Space>
		</div>
...
const VideoEditPanel = useMini(_VideoEditPanel)
