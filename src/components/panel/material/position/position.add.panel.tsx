...
import style from "@/lib/wangEditor/config/style"
...
const _PositionEdit: React.FC<Props> = props => {
	...
	const show = useMemo(() => panelState.current === "position" && panelState.action === "edit", [panelState, state])
	...
	useEffect(() => {
		setPositionItem({ config: { pic: "", tour: "" } })
	}, [])
	...
	const onFinish = useCallback(
		data => {
			...
			// setPositionData(positionData.concat(item))
			eventBus.emit("position.add", item)
			state.editHook &&
				dispatch({
					type: "set",
					payload: {
		...
		[state, positionItem, positionData]
	)
	// 操作定位数据
	// useEffect(() => {
						...
						>
							...
							<Form.Item name="classify" label="" rules={[]}>
								<Radio.Group buttonStyle="solid" onChange={changeClassList} style={{ width: 300 }}>
									{!!hasDefClass &&
										classList.map(c => (
											<Radio.Button value={c.id} style={{ margin: "10px" }} key={c.id}>
												{c.name}
												<Button
													type="text"
